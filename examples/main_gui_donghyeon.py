...
class MainWindow(QWidget):
    ...
    def put_img_to_labels(self, dq):
        ...
        image = self.dq[0]        
        image = cv2.resize(image, dsize=(0, 0), fx=0.4, fy=0.4, interpolation=cv2.INTER_LINEAR)
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        height, width, channel = image.shape
        step = channel * width
        qImg = QImage(image.data, width, height, step, QImage.Format_RGB888)
    ...
    def viewCam(self):
        ...
        fps = 1/(sec)
        FPS = "FPS : %0.1f" % fps
        cv2.putText(img, FPS, (0, 100), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 255, 0), thickness=5)
        ...
        image = cv2.resize(result_img, dsize=(0, 0), fx=0.4, fy=0.4, interpolation=cv2.INTER_LINEAR)
