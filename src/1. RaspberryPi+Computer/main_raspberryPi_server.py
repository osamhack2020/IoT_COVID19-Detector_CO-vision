...
while True:
    name, img = image_hub.recv_image() # 이미지를 지속적으로 전송하고 계속해서 전송받으므로 가시광선 이미지와 적외선 이미지가 교차하여 들어오는 것을 분류
    if name == 'visible_image' :
        rpi, img = image_hub.recv_image() #가시광선 카메라 현재화면 이미지 read
    elif name == 'thermal_image':
        rpi, thermal_image_data = image_hub.recv_image() #적외선 카메라 현재화면을 이미지로 grab
    ...
    if cv2.waitKey(1) == ord('q'):  # q누르면 동영상 종료
        break
    image_hub.send_reply(b'OK') #요청-응답 패턴을 사용하므로 지속적으로 수신 성공메시지를 보내야 함
out.release()
image_hub.close()
